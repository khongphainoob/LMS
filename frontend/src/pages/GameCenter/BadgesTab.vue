	...
	<div class="space-y-4">
		...
		<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
			<div v-for="b in filteredBadges" :key="b.name" class="border border-outline-gray-2 bg-surface-white rounded-xl shadow-sm overflow-hidden transition-all duration-300" :class="{ 'opacity-50': !b.earned }">
				...
				<div class="p-3"><div class="text-sm font-semibold text-ink-gray-9 truncate">{{ b.title }}</div><div class="text-[11px] text-ink-gray-5 mt-0.5 line-clamp-2">{{ b.description }}</div></div>
			</div>
		</div>
	</div>
</template>
