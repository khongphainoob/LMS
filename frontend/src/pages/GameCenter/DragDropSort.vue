<template>
	<div class="space-y-4">
		...
		<div v-else-if="!finished" class="border border-outline-gray-2 bg-surface-white rounded-xl p-5">
			...
			<div class="flex gap-2 mb-5 p-3 rounded-xl border-2 border-dashed transition-colors min-h-[56px] items-center" :class="isDragOver ? 'border-ink-blue-4 bg-blue-50 dark:bg-blue-900/10' : 'border-outline-gray-3 bg-surface-gray-1'" @dragover.prevent="isDragOver = true" @dragleave="isDragOver = false" @drop.prevent="onDrop">
...
function onItemDragStart(e, itemOrIdx, fromSorted) { let item; if (fromSorted) { item = sortedItems.value[itemOrIdx]; sortedItems.value.splice(itemOrIdx, 1) } else { item = itemOrIdx } e.dataTransfer.setData("text/plain", item.id) }
function onDrop() { isDragOver.value = false }
