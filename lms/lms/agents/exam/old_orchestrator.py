...
def generate_with_fallback(llm, prompt_text, schema_class, config=None):
    ...
    try:
        ...
        if "401" in str(e) or "404" in str(e) or "AuthenticationError" in str(e):
            raise e
    return None
...
def node_blueprint(state: ExamState, config: RunnableConfig):
    ...
    try:
        blueprint_dict = generate_with_fallback(llm, prompt, ExamBlueprintSchema, config=config)
        if blueprint_dict:
...
def node_generate(state: ExamState, config: RunnableConfig):
    """Phase 3: Final Generation (Iterative Section Writer)"""
    llm, _, _ = get_llm("exam_generator", temperature=0.4, max_tokens=8192)
    state_config = state["teacher_config"]
    ...
    for section_blueprint in state["blueprint"].get("sections_blueprint", []):
        ...
        try:
            section_dict = generate_with_fallback(llm, prompt, ExamSectionSchema, config=config)
            if section_dict:
        ...
        if doc.owner:
            try:
                ...
                notification = frappe._dict({
                    "subject": f"📋 Khung đề thi cần duyệt: {doc.title}",
                    "email_content": (
                        f"Khung đề thi <b>{doc.title}</b> đã được AI phân tích xong và đang chờ bạn phê duyệt.<br><br>"
                        f"Vui lòng kiểm tra và duyệt để hệ thống tiếp tục sinh câu hỏi."
                    ),
                    "for_user": doc.owner,
                ...
                make_notification_logs(notification, [doc.owner])
                frappe.db.commit()
            except Exception as e:
                logger.error(f"Failed to send immediate review notification: {e}")
...
def process_phase_1_5_regenerate(exam_name: str, feedback: str):
    ...
    try:
        ...
        if not state:
            raise Exception("Exam state not found.")
            
        current_instructions = state.get("teacher_config", {}).get("teacher_instructions", "")
        if feedback:
            state["teacher_config"]["teacher_instructions"] = current_instructions + f"\n\n[USER FEEDBACK FOR REGENERATION]: {feedback}"
            
        thread_id = f"exam_{exam_name}"
        config = get_unified_config_dict(agent_name="exam_phase_1_5", session_id=thread_id)
        
        state["errors"] = []
        state = node_blueprint(state, config)
        
        if state.get("errors"):
            raise Exception(" | ".join(state["errors"]))
            
        save_state_to_file(exam_name, state)
        ...
        logger.error(f"AI Exam Phase 1.5 Error: {e}", exc_info=True)
        frappe.db.set_value("AI Exam", exam_name, "status", "Failed")
        frappe.db.set_value("AI Exam", exam_name, "error_log", str(e))
        frappe.publish_realtime("ai_exam_update", {"name": exam_name}, room=exam_name)
