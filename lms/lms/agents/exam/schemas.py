...
class MediaAssetSchema(BaseModel):
    asset_type: Literal["latex", "plot", "geometry", "image"] = Field(description="Type of media asset")
    content: str = Field(description="The actual content: LaTeX string, Base64 image, or SVG string")
...
class ExamQuestionSchema(BaseModel):
    ...
    question_text: str = Field(description="The question content. Should include placeholder [MEDIA_1] if media is attached.")
    options: Optional[List[OptionSchema]] = Field(default=None, description="Options for Multiple Choice questions")
    ...
    difficulty_level: Literal["Nhận biết", "Thông hiểu", "Vận dụng", "Vận dụng cao"] = Field(description="Difficulty level")
    ...
    needs_visual: bool = Field(default=False, description="True if this question requires geometric figures, data plots, or complex statistical charts")
    visual_description: Optional[str] = Field(default=None, description="Detailed description of the visual needed (if needs_visual is True)")
    media_assets: Optional[List[MediaAssetSchema]] = Field(default=None, description="Generated media assets")
...
class ExamBlueprintSchema(BaseModel):
    ...
    sections_blueprint: List[dict] = Field(description="Outline of sections and number of questions per section")

class ExamSchema(BaseModel):
    title: str = Field(description="Title of the exam")
    instructions: str = Field(description="General instructions for the exam")
    sections: List[ExamSectionSchema] = Field(description="List of sections containing the final questions")
